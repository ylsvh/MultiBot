...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('githubrepo')
        .setDescription('Affiche les informations d’un dépôt GitHub.')
        .addStringOption(option =>
            option
                .setName('repository')
                .setDescription('Pseudo/nom-repo ou lien GitHub')
                .setRequired(true)
        ),
    ...
    async execute(interaction) {
        ...
        try {
            ...
            if (
                ...
                if (
                    ...
                    return interaction.reply({
                        content: '❌ Le lien fourni n\'est pas un lien GitHub valide.',
                        ephemeral: true
                    });
                }
                ...
                if (path.length < 2 || !path[0] || !path[1]) {
                    return interaction.reply({
                        content: '❌ Le lien GitHub ne contient pas de dépôt valide.',
                        ephemeral: true
                    });
                }
            ...
            if (response.status === 404) {
                return interaction.reply({
                    content: '❌ Ce dépôt GitHub n\'existe pas ou est privé.',
                    ephemeral: true
                });
            }
            ...
            const formatNumber = number =>
                Number(number || 0).toLocaleString('fr-FR');

            const language =
                repo.language || 'Aucun langage renseigné';

            const license =
                repo.license?.name || 'Aucune licence';
