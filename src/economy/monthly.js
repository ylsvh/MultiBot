...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('monthly')
    .setDescription('Récupère la récompense mensuelle.'),
  ...
  async execute(interaction, client) {
    ...
    if (now - lastMonthly < MONTHLY_COOLDOWN) {
      ...
      container.addTextDisplayComponents(
        new TextDisplayBuilder().setContent(
          ...
          `Prochain monthly : <t:${Math.floor(
            (lastMonthly + MONTHLY_COOLDOWN) / 1000
          )}:R>`
        )
      );
    ...
    container.addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        `💎 Nouveau total : **${economy
          .getUserData(interaction.user.id)
          .cash.toLocaleString()} coins**\n` +
        `⏰ Prochain monthly : <t:${Math.floor(
          (now + MONTHLY_COOLDOWN) / 1000
        )}:R>`
      )
    );
